...
import { webSpeechTTS, googleTranslateTTS, azureTTS } from './free-tts-providers.js';
...
class App {
    constructor() {
        ...
        this.translationMode = 'free'; // 'free' | 'soniox' | 'local'
    ...
    async init() {
        ...
        for (const tts of [elevenLabsTTS, edgeTTSRust, googleTTS, googleTranslateTTS, azureTTS]) {
    ...
    async _checkPlatformSupport() {
        ...
        if (!this.isAppleSilicon) {
            // Hide Local MLX option (requires Apple Silicon)
            ...
            // If user had local mode, switch to free (not soniox)
            const settings = settingsManager.get();
            if (settings.translation_mode === 'local') {
                settings.translation_mode = 'free';
    ...
    _bindEvents() {
        ...
        // Web Speech rate slider
        document.getElementById('range-webspeech-rate')?.addEventListener('input', (e) => {
            const label = document.getElementById('webspeech-rate-value');
            if (label) label.textContent = parseFloat(e.target.value).toFixed(1) + 'x';
        });

        // Azure speed slider
        document.getElementById('range-azure-speed')?.addEventListener('input', (e) => {
            const label = document.getElementById('azure-speed-value');
            if (label) label.textContent = parseInt(e.target.value) + '%';
        });

        // Azure key toggle
        document.getElementById('btn-toggle-azure-key')?.addEventListener('click', () => {
            const input = document.getElementById('input-azure-tts-key');
            if (input) input.type = input.type === 'password' ? 'text' : 'password';
        });

        // Web Speech lang change — repopulate voices
        document.getElementById('select-webspeech-lang')?.addEventListener('change', () => {
            this._populateWebSpeechVoices();
        });
    ...
    _populateSettingsForm() {
        ...
        document.getElementById('select-translation-mode').value = s.translation_mode || 'free';
        this._updateModeUI(s.translation_mode || 'free');
        const wmSel = document.getElementById('select-whisper-model');
        if (wmSel) wmSel.value = s.whisper_model || 'small';
        ...
        // Web Speech settings
        const wsLangSel = document.getElementById('select-webspeech-lang');
        if (wsLangSel) wsLangSel.value = s.webspeech_lang ? s.webspeech_lang.slice(0,2) : 'vi';
        const wsRateSlider = document.getElementById('range-webspeech-rate');
        if (wsRateSlider) { wsRateSlider.value = s.webspeech_rate || 1.1; document.getElementById('webspeech-rate-value').textContent = (s.webspeech_rate || 1.1) + 'x'; }
        // Google Translate TTS settings
        const gtLangSel = document.getElementById('select-gtranslate-lang');
        if (gtLangSel) gtLangSel.value = s.gtranslate_lang || 'vi';
        // Azure TTS settings
        const azKeyInput = document.getElementById('input-azure-tts-key');
        if (azKeyInput) azKeyInput.value = s.azure_tts_key || '';
        const azRegionInput = document.getElementById('select-azure-region');
        if (azRegionInput) azRegionInput.value = s.azure_tts_region || 'eastus';
        const azVoiceSel = document.getElementById('select-azure-voice');
        if (azVoiceSel) azVoiceSel.value = s.azure_tts_voice || 'vi-VN-HoaiMyNeural';
        const azSpeedSlider = document.getElementById('range-azure-speed');
        if (azSpeedSlider) { azSpeedSlider.value = s.azure_tts_speed !== undefined ? s.azure_tts_speed : 0; document.getElementById('azure-speed-value').textContent = (s.azure_tts_speed || 0) + '%'; }
    }
    ...
    async _saveSettingsFromForm() {
        ...
        // TTS settings
        settings.whisper_model = document.getElementById('select-whisper-model')?.value || 'small';
        settings.tts_provider = document.getElementById('select-tts-provider')?.value || 'edge';
        settings.webspeech_voice = document.getElementById('select-webspeech-voice')?.value || null;
        settings.webspeech_lang = (document.getElementById('select-webspeech-lang')?.value || 'vi') + '-VN';
        settings.webspeech_rate = parseFloat(document.getElementById('range-webspeech-rate')?.value || 1.1);
        settings.gtranslate_lang = document.getElementById('select-gtranslate-lang')?.value || 'vi';
        settings.azure_tts_key = document.getElementById('input-azure-tts-key')?.value.trim() || '';
        settings.azure_tts_region = document.getElementById('select-azure-region')?.value || 'eastus';
        settings.azure_tts_voice = document.getElementById('select-azure-voice')?.value || 'vi-VN-HoaiMyNeural';
        settings.azure_tts_speed = parseInt(document.getElementById('range-azure-speed')?.value || 0);
    ...
    _toggleTTS() {
        ...
        if (provider === 'azure' && !settings.azure_tts_key) {
            this._showToast('Add Azure Subscription Key in Settings → TTS', 'error');
            this._showView('settings');
            return;
        }
        ...
        if (this.ttsEnabled) {
            ...
            const label = {
                ...
                webspeech: 'Web Speech (Free)',
                gtranslate: 'Google Translate TTS (Free)',
    ...
    _getActiveTTS() {
        ...
        if (provider === 'webspeech') return webSpeechTTS;
        if (provider === 'gtranslate') return googleTranslateTTS;
        if (provider === 'azure') return azureTTS;
        return edgeTTSRust;
    }
    ...
    _configureTTS(tts, settings) {
        ...
        if (provider === 'elevenlabs') {
            ...
            tts.configure({
                voiceName: settings.webspeech_voice || null,
                lang: settings.webspeech_lang || 'vi-VN',
                rate: settings.webspeech_rate !== undefined ? settings.webspeech_rate : 1.1,
            });
        } else if (provider === 'gtranslate') {
            tts.configure({
                lang: settings.gtranslate_lang || 'vi',
            });
        } else if (provider === 'azure') {
            tts.configure({
                subscriptionKey: settings.azure_tts_key,
                region: settings.azure_tts_region || 'eastus',
                voice: settings.azure_tts_voice || 'vi-VN-HoaiMyNeural',
                lang: (settings.azure_tts_voice || 'vi-VN-HoaiMyNeural').split('-').slice(0,2).join('-'),
                rate: settings.azure_tts_speed !== undefined ? settings.azure_tts_speed : 0,
            });
        } else {
            tts.configure({
    ...
    _updateTTSProviderUI(provider) {
        const panels = ['edge', 'google', 'elevenlabs', 'webspeech', 'gtranslate', 'azure'];
        panels.forEach(p => {
            const el = document.getElementById(`tts-${p}-settings`);
            if (el) el.style.display = provider === p ? '' : 'none';
        });
        // Populate Web Speech voice list dynamically
        ...
        if (hint) {
            const hints = {
                edge: '🆓 Miễn phí hoàn toàn — giọng tự nhiên, không cần API key',
                google: '💰 Chất lượng cao — cần Google Cloud API key (1M ký tự/tháng miễn phí)',
                elevenlabs: '💳 Chất lượng vượt trội — cần ElevenLabs API key (trả phí)',
                webspeech: '🆓 Miễn phí hoàn toàn — dùng giọng đọc có sẵn trong hệ thống',
                gtranslate: '🆓 Miễn phí hoàn toàn — dùng giọng Google Translate (không cần đăng ký)',
                azure: '🆓 500.000 ký tự/tháng miễn phí — cần tài khoản Azure (không cần thẻ tín dụng)',
            };
    ...
    _populateWebSpeechVoices() {
        const select = document.getElementById('select-webspeech-voice');
        if (!select) return;
        const populate = () => {
            const voices = speechSynthesis.getVoices();
            if (voices.length === 0) return;
            const currentLang = document.getElementById('select-webspeech-lang')?.value || 'vi';
            const filtered = voices.filter(v => v.lang.toLowerCase().startsWith(currentLang));
            const allGroup = voices.filter(v => !v.lang.toLowerCase().startsWith(currentLang));
            select.innerHTML = '';
            if (filtered.length > 0) {
                const grp = document.createElement('optgroup');
                grp.label = `Giọng ngôn ngữ đích (${currentLang.toUpperCase()})`;
                filtered.forEach(v => {
            ...
            if (allGroup.length > 0) {
                ...
                grp2.label = 'Tất cả giọng khác';
                allGroup.slice(0, 20).forEach(v => {
                    const opt = document.createElement('option');
                    opt.value = v.name;
                    opt.textContent = `${v.name} (${v.lang})`;
            ...
            if (select.options.length === 0) {
                const opt = document.createElement('option');
                opt.value = '';
                opt.textContent = '(Không tìm thấy giọng — thử ngôn ngữ khác)';
    ...
    _updateModeUI(mode) {
        ...
        // Hint texts
        const hints = ['free', 'soniox', 'local'];
        hints.forEach(m => {
            const el = document.getElementById(`hint-mode-${m}`);
            if (el) el.style.display = mode === m ? '' : 'none';
        });

        // Soniox-only sections
        ...
        // Free pipeline settings
        const sectionFree = document.getElementById('section-free-settings');
        if (sectionFree) sectionFree.style.display = isFree ? '' : 'none';
    }
    ...
    async start() {
        ...
        if (this.translationMode === 'soniox' && !settings.soniox_api_key) {
            this._showToast('Soniox API key là bắt buộc cho chế độ Cloud. Vào Settings để thêm.', 'error');
            this._showView('settings');
            return;
        }
        ...
        if (this.translationMode === 'free') {
            await this._startFreeMode(settings);
    ...
    async _startFreeMode(settings) {
        console.log('[App] Starting Free mode (faster-whisper + MyMemory)...');
        this._updateStatus('connecting');
        ...
        try {
            ...
            this.localPipelineChannel.onmessage = (msg) => {
                ...
                catch (e) { console.warn('[Free] parse error:', msg); return; }
                try { this._handleLocalPipelineResult(data); }
                catch (e) { console.error('[Free] handler error:', e); }
            };

            const whisperModel = settings.whisper_model || 'small';
            const sourceLang = settings.source_language || 'auto';
            const targetLang = settings.target_language || 'vi';

            this._showToast(`Đang tải Whisper (${whisperModel})... lần đầu có thể mất vài phút`, 'success');

            await invoke('start_free_pipeline', {
                sourceLang: sourceLang,
                targetLang: targetLang,
                whisperModel: whisperModel,
                channel: this.localPipelineChannel,
            });
            console.log('[App] Free pipeline spawned');
        } catch (err) {
            console.error('Free pipeline error:', err);
            this._showToast(`Lỗi Free Pipeline: ${err}`, 'error');
            await this.stop();
            return;
        }
        ...
        try {
            ...
            audioChannel.onmessage = (msg) => {
                if (typeof msg === 'string' && msg.startsWith('AUDIO:')) {
                    const base64 = msg.slice(6);
                    this._sendAudioToFreePipeline(base64);
                }
            };
            await invoke('start_capture', { source: this.currentSource, channel: audioChannel });
            console.log('[App] Audio capture started (free mode)');
        } catch (err) {
            console.error('Audio capture error:', err);
            this._showToast(`Audio capture error: ${err}`, 'error');
            await this.stop();
        }
    }

    async _sendAudioToFreePipeline(base64Audio) {
        try {
            await invoke('send_audio_to_pipeline', { audioBase64: base64Audio });
        } catch (err) {
            // Ignore minor errors during streaming
        }
    }
    ...
    async stop() {
        ...
        if (this.translationMode === 'free' || this.translationMode === 'local') {
            // Stop local pipeline
            try {
    ...
    async _saveTranscriptFile() {
        ...
        const content = this.transcriptUI.getFormattedContent({
            model: this.translationMode === 'soniox' ? 'Soniox Cloud API' : (this.translationMode === 'free' ? 'Free Whisper + MyMemory' : 'Local MLX Whisper'),
            sourceLang,
            targetLang,
            duration,
